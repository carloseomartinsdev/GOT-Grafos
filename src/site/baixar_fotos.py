import requests
import os
import csv
from bs4 import BeautifulSoup
import time

def download_from_google(char_name):
    """Busca '[nome] GOT' no Google Images e baixa primeira foto"""
    
    # Verifica se foto já existe (nome em caixa alta)
    filename = f"public/resources/imagens/personagens/{char_name.replace(' ', '_').replace('/', '_').upper()}.JPG"
    if os.path.exists(filename):
        return 'exists'
    
    try:
        # Monta URL de busca do Google Images
        search_query = f"{char_name} game of thrones"
        url = f"https://www.google.com/search?q={search_query}&tbm=isch"
        
        # Headers para simular navegador
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'
        }
        
        # Faz requisição
        response = requests.get(url, headers=headers, timeout=10)
        
        if response.status_code == 200:
            soup = BeautifulSoup(response.text, 'html.parser')
            
            # Procura primeira imagem
            img_tags = soup.find_all('img')
            
            for img in img_tags[1:]:  # Pula logo do Google
                img_url = img.get('src') or img.get('data-src')
                
                if img_url and img_url.startswith('http'):
                    # Baixa imagem
                    img_data = requests.get(img_url, timeout=10).content
                    
                    # Salva
                    with open(filename, 'wb') as f:
                        f.write(img_data)
                    
                    return 'success'
        
        return 'failed'
    
    except Exception as e:
        return 'failed'

def main():
    # Carrega personagens do dicionário
    characters = []
    with open('datasets/personagens_dicionario.csv', 'r', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        for row in reader:
            characters.append(row['NOME_OFICIAL'])
    
    # Cria pasta
    os.makedirs('public/resources/imagens/personagens', exist_ok=True)
    
    print(f"Buscando fotos no Google Images...")
    print("="*60)
    print(f"Total de personagens: {len(characters)}")
    print("="*60)
    
    success = 0
    exists = 0
    failed = 0
    
    for i, char in enumerate(characters, 1):
        print(f"[{i}/{len(characters)}] {char}...", end=' ')
        
        result = download_from_google(char)
        sleep_time = 0
        
        if result == 'success':
            print("OK")
            sleep_time = 2
            success += 1
        elif result == 'exists':
            print("JA EXISTE")
            sleep_time = 0
            exists += 1
        else:
            print("FALHOU")
            sleep_time = 0
            failed += 1
        
        time.sleep(sleep_time)
    
    print("="*60)
    print(f"Concluido!")
    print(f"  Baixadas: {success}")
    print(f"  Ja existiam: {exists}")
    print(f"  Falharam: {failed}")
    print(f"  Total: {success + exists}/{len(characters)}")
    print(f"Pasta: public/resources/imagens/personagens/")

if __name__ == '__main__':
    main()
